(function($){
  ...
  function mggScrollImg(box,config){
    ...
    this.auto_wait_time = this.config.auto_wait_time;//轮播间隔
    ...
    this.index = 0;
    this.firstIndex = this.config.firstIndex;
    this.busy = false;
    this.timer;
    this.init();
  }
  $.extend(mggScrollImg.prototype,{
    init : function(){
      ...
      this.init_loop();
      this.auto_scroll();
      this._initIndex();//added by zdh at 2016.07.19
    },
    _initIndex : function(){
      var self = this;
      //不循环不自动播放的时候滚动到指定位置
      if(!self.loop && !self.auto){
        self.go_index(self.firstIndex);
      }
    },
    bind_event : function(){
      var self = this;
      self.box.bind('touchstart',function(e){
        ...
        if(e.touches.length==1 && !self.busy){
          return self.move(e.touches[0].screenX,e.touches[0].screenY);//这里根据返回值决定是否阻止默认touch事件
        }
      }).bind('touchend',function(e){
        !self.busy && self.move_end();
      });
      //console.log("bind_event finished");
    },
    /*
